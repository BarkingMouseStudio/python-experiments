...
class App(ShowBase):
    ...
    def update(self, task):
        ...
        animation_frame = int(frame_count / 10)
        self.animated_rig.pose('walk', animation_frame % self.num_frames)

        # apply root motion
        if animation_frame % self.num_frames == 0:
            match_pose(self.animated_joint_list, self.control_joint_list)
            apply_control_joints(self.control_joint_list, self.joint_list)
        else:
            match_pose(self.animated_joint_list[:1], self.control_joint_list[:1])
            apply_control_joints(self.control_joint_list[:1], self.joint_list[:1])
        ...
        next_joint_positions = [node.get_pos(parent) if parent is not None else node.get_pos(self.animated_rig) for node, parent in self.animated_joint_list]
        next_joint_rotations = [node.get_hpr(parent) if parent is not None else node.get_hpr(self.animated_rig) for node, parent in self.animated_joint_list]
        ...
        Y = self.model.predict(np.array([X]))[0] * 3.0
        ...
        err = measure_error(self.joint_list, self.animated_joint_list)
